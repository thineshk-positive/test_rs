...
# def save_markdown(md_text: str, pdf_path: str, output_dir: str):
...
# ============================================================
# PDF ? Text using PyMuPDF (Fallback Method)
# ============================================================
# def pdf_to_text_fitz(pdf_path):
    # """
    # Fallback extraction method using PyMuPDF (fitz).
    # Returns plain text extracted from all pages.
    # """
    # print("?? Using fallback extraction method (PyMuPDF/fitz)...")
...
def extract_endorsements(md_text: str, debug: bool = False):
    ...
    # Enhanced pattern to catch various endorsement formats
    heading_pattern = re.compile(
        r"^\s*\*{0,2}\s*(?:Endorsement|Endt\.?|Endoresemt)\.?\s*No\.?\s*"
        r"(\d+)"
...
# ============================================================
# Extract endorsements with fallback to fitz
# ============================================================
# def extract_endorsements_with_fallback(pdf_path: str, debug: bool = False):
    # """
    # Try primary method first, if no endorsements found, use fitz fallback.
    # Also saves extracted markdown/text to disk for debugging.
    # """
    # base_name = os.path.splitext(os.path.basename(pdf_path))[0]
    # output_dir = os.path.dirname(pdf_path)

    # # ============================================================
    # # Primary method: pymupdf4llm (Markdown)
    # # ============================================================
    # md_text = pdf_to_md(pdf_path)

    # # ?? SAVE PRIMARY MARKDOWN
    # md_path = os.path.join(output_dir, f"{base_name}_primary.md")
    # with open(md_path, "w", encoding="utf-8") as f:
        # f.write(md_text)

    # if debug:
        # print(f"?? Primary markdown saved at: {md_path}")

    # endorsements = extract_endorsements(md_text, debug)

    # if endorsements:
        # print(f"? Found {len(endorsements)} endorsements using primary method")
        # return endorsements, md_text

    # ============================================================
    # Fallback method: PyMuPDF (plain text ? saved as .md)
    # ============================================================
    # print("?? No endorsements found with primary method. Trying fallback...")
    # fitz_text = pdf_to_text_fitz(pdf_path)

    # # ?? SAVE FALLBACK TEXT
    # fallback_md_path = os.path.join(output_dir, f"{base_name}_fallback.md")
    # with open(fallback_md_path, "w", encoding="utf-8") as f:
        # f.write(fitz_text)

    # if debug:
        # print(f"?? Fallback markdown saved at: {fallback_md_path}")

    # endorsements = extract_endorsements(fitz_text, debug)

    # if endorsements:
        # print(f"? Found {len(endorsements)} endorsements using fallback method")
        # return endorsements, fitz_text
    # else:
        # print("? No endorsements found even with fallback method")
        # return {}, fitz_text
...
# ============================================================
# Get endorsements content (UPDATED with fallback)
# ============================================================
def get_endorsements_content(pdf_path, desired_endorsements):
    # Use the new fallback-enabled extraction
    endorsements, extracted_text = extract_endorsements_with_fallback(pdf_path)
    ...
    special_conditions_context = overaly_specail_condtion(extracted_text)
    return list(matching_endorsements.values()), special_conditions_context
    
def get_endorsements_content_from_md(md_path, desired_endorsements):
...
# def generate_eligibility(pdf_path: str, chat, source_dir):
    # # ============================================================
    # # 1?? Extract Endorsement 1-series (Relationship Info)
    # # ============================================================
    # endt1_texts, special_conditions = get_endorsements_content(pdf_path, ["1", "1a", "1b", "1c", "1d"])
    
    # if not endt1_texts:
        # print("? No 1-series endorsements found even with fallback method.")
        # print("?? Cannot proceed without endorsement data.")
        # return
    ...
    # # ============================================================
    # # 2?? Extract Endorsement 10-series (Buffer + CI Info)
    # # ============================================================
    ...
    # if endt10_texts:
        ...
        # )
    # else:
        # # ?? 10-series not found - check if Special Condition still has CI info
        ...
        # if special_condition_text and "critical illness" in special_condition_text.lower():
            ...
            # )
        # else:
            # print("?? No 10-series endorsement or relevant Critical Illness context found.")
            # buffer_json = {
                # "Corporate Buffer Eligibility": {
                    # "Corporate Buffer applicable": "No",
                    # "Buffer Type": "",
    ...
    # ============================================================
    # 3?? Save Combined Excel Output
    # ============================================================
    #save_combined_to_excel(rel_json, buffer_json, pdf_path, source_dir)
def generate_eligibility_from_md(md_path: str, chat, source_dir):
    fake_pdf_path = md_path.replace(".md", ".pdf")  # only for Excel name

    # 1️⃣ Relationship (1-series)
    endt1_texts, special_conditions = get_endorsements_content_from_md(
        md_path, ["1", "1a", "1b", "1c", "1d"]
    )

    if not endt1_texts:
        print("❌ No 1-series endorsements found in markdown.")
        return
    ...
    # 2️⃣ Buffer + CI (10-series)
    endt10_texts, special_conditions_10 = get_endorsements_content_from_md(
        md_path, ["10", "10a", "10b", "10c", "10d"]
    )
    ...
    if endt10_texts:
        ...
        sc_text = special_conditions_10.get("special condition", "")
        if "critical illness" in sc_text.lower():
            buffer_json = buffer_and_ci_extractor(sc_text, chat, sc_text)
        else:
            buffer_json = {
                "Corporate Buffer Eligibility": {
                    "Corporate Buffer applicable": "No",
    ...
    save_combined_to_excel(rel_json, buffer_json, fake_pdf_path, source_dir)
...
if __name__ == "__main__":
    ...
    #pdf_path = "/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/work_dir/HG00008059000100.pdf"
    source_dir = r"/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/work_dir/"
    md_path = r"/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/work_dir/cleaned_data.md"
    print(md_path)
    #generate_eligibility_from_md(pdf_path, chat, source_dir)
    generate_eligibility_from_md(md_path, chat, source_dir)
...
# ============================================================
# PDF ? Markdown (Modified to read MD files directly)
# ============================================================
def pdf_to_md(file_path):
    """
    Reads markdown content from either PDF or MD file.
    If the file is .md, read it directly.
    If the file is .pdf, convert it using pymupdf4llm.
    """
    if file_path.lower().endswith('.md'):
        print(f"?? Reading markdown file directly: {file_path}")
...
# ============================================================
# Clean endorsement content by removing member data tables
# ============================================================
...
# ============================================================
# Get endorsements content (UPDATED - only extracts once)
# ============================================================
def get_all_endorsements(file_path):
    """
...
def get_specific_endorsements(endorsements_dict, desired_ids):
    ...
    if not matching_endorsements:
        print("??  No matching endorsements found!")

    return list(matching_endorsements.values())
...
def save_combined_to_excel(rel_json: dict, buffer_json: dict, file_path: str, source_dir):
    ...
    # If source_dir is not specified, use the same directory as the input file
    ...
    # Extract base name from file path (works for both .pdf and .md)
    base_name = os.path.splitext(os.path.basename(file_path))[0]
    excel_file = os.path.join(output_folder, f"{base_name}_eligibility_sheet.xlsx")
...
def generate_eligibility(file_path: str, chat, source_dir):
    """
    Modified to accept both PDF and MD files.
    """
    print(f"\n{'='*60}")
    print(f"?? Starting eligibility generation for: {file_path}")
    print(f"{'='*60}\n")
    
    # ============================================================
    # Extract ALL endorsements once
    # ============================================================
    print("?? Extracting all endorsements...")
    all_endorsements, special_conditions = get_all_endorsements(file_path)
    
    # ============================================================
    # 1?? Extract Endorsement 1-series (Relationship Info)
    # ============================================================
    endt1_texts = get_specific_endorsements(all_endorsements, ["1", "1a", "1b", "1c", "1d"])
    if not endt1_texts:
    ...
    # ============================================================
    endt10_texts = get_specific_endorsements(all_endorsements, ["10", "10a", "10b", "10c", "10d"])
    ...
    if endt10_texts:
        ...
        special_condition_text = special_conditions.get('special condition', '').strip()
    ...
    # ============================================================
    save_combined_to_excel(rel_json, buffer_json, file_path, source_dir)

# ============================================================
# Auto-generate MD path based on PDF input name
# ============================================================
def generate_md_path_from_pdf(pdf_path: str, base_output_dir: str = "/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/UAT/work_dir/") -> str:
    """
    ...
    #file_base_name = pdf_file.stem
    
    # Construct the MD path following the pattern
    #md_path = Path(base_output_dir) / f"{file_base_name}_1" / file_base_name / "auto" / f"{file_base_name}.md"
...
# ============================================================
# Run - MODIFIED TO AUTO-GENERATE MD PATH
# ============================================================
if __name__ == "__main__":
    # Option 1: Directly provide PDF path - MD path will be auto-generated
    pdf_input_path = "/home/ubuntu/rspdf/input_pdfs/HG00008059000100.pdf"
    
    # Option 2: Or provide MD path directly (if you already have it)
    # md_input_path = "/home/ubuntu/rspdf/newtest/1&2&3/HG00008059000100_1/HG00008059000100/auto/HG00008059000100.md"
    #"/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/UAT/work_dir/mineru_temp/HG00008059000100/auto/HG00008059000100.md"
    
    # Determine which path to use
    if pdf_input_path:
        # Auto-generate MD path from PDF
        md_path = generate_md_path_from_pdf(pdf_input_path)
        print(f"?? Auto-generated MD path: {md_path}")
    else:
        # Use provided MD path
        md_path = md_input_path
    
    #source_dir = os.path.dirname(md_path)
    source_dir = "/home/ubuntu/THINESH_WS/royal_sunadaram_hscope/UAT/work_dir"
    
    # Verify the file exists
    if not os.path.exists(md_path):
        print(f"? Error: File not found at {md_path}")
        print(f"Please ensure the MinerU conversion has completed successfully.")
    else:
        print(f"? Found file: {md_path}")
        generate_eligibility(md_path, chat, source_dir)